        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post" class="row">
                        ...
                        <select name="barang_id" id="barang_id" class="form-control" required>
                            ...
                            <?php if (!empty($barang_list) && mysqli_num_rows($barang_list) > 0): ?>
                                ...
                                <?php while ($row = mysqli_fetch_assoc($barang_list)): ?>
                                    <option value="<?= htmlspecialchars($row["id"]) ?>" <?= (!empty($edit_data) && $edit_data['barang_id'] == $row["id"]) ? 'selected' : '' ?>>
                    ...
                    <div class="col-md-4 col-12">
                        <input type="number" name="quantity" class="form-control" placeholder="Jumlah" min="0" required value="<?= !empty($edit_data) ? (int)$edit_data['quantity'] : '' ?>">
                    </div>
                    <div class="<?= !empty($edit_data) ? 'col-6' : 'col-12' ?> mt-2">
                        <button type="submit" class="btn btn-primary w-100"><?= !empty($edit_data) ? 'Update Barang' : 'Tambah Barang Baru' ?></button>
                    </div>
                    <?php if (!empty($edit_data)): ?>
...
<div class="mb-3">
    <form method="post" class="row">
        ...
        <input type="hidden" name="<?= !empty($edit_data) ? 'edit' : 'tambah' ?>" value="1">
        <div class="col-md-4 col-12">
            <input type="text" name="nama_barang" class="form-control" placeholder="Nama Barang" required value="<?= !empty($edit_data) ? htmlspecialchars($edit_data['nama_barang']) : '' ?>">
        </div>
        <div class="col-md-4 col-12">
            <select name="kategori_id" class="form-control" required>
                <option value="">Pilih Kategori</option>
                <?php foreach ($kategori_list as $kat): ?>
                    <option value="<?= $kat['id'] ?>" <?= (!empty($edit_data) && $edit_data['kategori_id'] == $kat['id']) ? 'selected' : '' ?>>
...
<section>
    <div class="table-responsive">
        <table class="table table-bordered table-striped">
            ...
            <tbody>
                <?php if (!empty($barang_list) && mysqli_num_rows($barang_list) > 0): ?>
                    <?php $no = 1; mysqli_data_seek($barang_list, 0); ?>
                    <?php while ($row = mysqli_fetch_assoc($barang_list)): ?>
                        <tr>
                            <td><?= $no++; ?></td>
